...
graph TD
    ...
    subgraph External ["External Services"]
        Groq["Groq Cloud LLM API (GPT-OSS 120B / Qwen 3.6 27B / GPT-OSS 20B)"]
...
* **Equal Load Balancing & Model Pools**:
  * **Flagship & Large Models**: `openai/gpt-oss-120b`, `qwen/qwen3.6-27b`.
  * **Fast & High-Throughput Models**: `openai/gpt-oss-20b` (30k TPM).
  * **Equal Load Balancing Strategy**: All document chunks and independent generation requests are dynamically striped in an atomic round-robin sequence across active models (`openai/gpt-oss-120b`, `qwen/qwen3.6-27b`, `openai/gpt-oss-20b`), ensuring inference volume is distributed equally and preventing single-model rate limits.
...
2. **TPM Rate Limit Bypass**: Multi-chunk document processing and individual requests dynamically distribute load equally across active model pools (`openai/gpt-oss-120b`, `qwen/qwen3.6-27b`, `openai/gpt-oss-20b`), keeping per-model token requests well within free-tier rate limits.
